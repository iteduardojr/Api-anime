...
const index = () => {
  ...
  const [registro, setAnotacao] = useState([])
  ...
  function getALL() {
    axios.get('/api/anotacoes/').then(resultado => {
      setAnotacao(resultado.data);
    })
  }
    ...
    function excluir(id) {
      ...
      swalWithBootstrapButtons.fire({
        ...
        if (result.isConfirmed) {
          axios.delete(`/api/anotacoes/${id}`).then(() => {
  ...
  return (
    <>
      <Cabecalho />
      <Container>
        <Link href='/anotacao/form' className="btn btn-primary mb-2 mt-5" ><IoAddOutline />Novo</Link>
        ...
        <Table striped bordered hover variant="dark">
          ...
          <tbody>
            {registro ? (registro.map((item, i) => (
              <tr key={item.id}>
                <td>
                  <Link href={'/anotacao/' + item.id} className='btn btn-warning'>
